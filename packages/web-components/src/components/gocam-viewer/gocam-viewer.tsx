...
export class GocamViewer {
  ...
  @Event({ eventName: "nodeOver", cancelable: true, bubbles: true })
  nodeOver: EventEmitter;
  @Event({ eventName: "nodeOut", cancelable: true, bubbles: true })
  nodeOut: EventEmitter;
  @Event({ eventName: "nodeClick", cancelable: true, bubbles: true })
  nodeClick: EventEmitter;
  @Event({ eventName: "layoutChange", cancelable: true, bubbles: true })
  ...
  @Watch("gocamId")
  @Watch("apiUrl")
  watchGoCamIdAndApiUrl(newValue: string, oldValue: string) {
  ...
  renderGoCam(cam: Cam, expandComplex = false, layout = "dagre") {
    ...
    cam.activities.forEach((activity: Activity) => {
      if (activity.visible) {
        ...
        if (activity.activityType === ActivityType.molecule) {
          ...
          el = this.createComplex(activity, expandComplex);
        } else {
          el = this.createNode(activity);
        }
        nodes.push(...el);
      }
    });
    ...
    cam.causalRelations.forEach((triple: Triple<Activity>) => {
      if (
        triple.predicate.visible &&
        triple.isTripleComplete() &&
        this.configService.isAllowedEdge(triple.predicate.edge.id)
      ) {
        const source = triple.predicate.isReverseLink
          ? triple.object
          : triple.subject;
        const target = triple.predicate.isReverseLink
          ? triple.subject
          : triple.object;
        const rglyph = glyph(triple.predicate.edge.id);
    ...
    this.renderCytoscape(elements, layout);
  }
  ...
  createComplex(activity: Activity, expandComplex = false) {
    ...
    if (expandComplex && edges) {
      ...
      if (gps.length > truncatedGps.length) {
        geneString +=
          " and " + (gps.length - truncatedGps.length).toString() + " more";
      }
      el = {
        group: "nodes",
        data: {
  ...
  renderCytoscape(elements, layout) {
  ...
  highlight(nodeId) {
    const sel = this.cy.elements('[id="' + nodeId + '"]');
    if (sel.size() > 0) {
  ...
  onMouseOver(evt) {
    ...
    if (evt.target && evt.target.length) {
      const elt = document.getElementById("gp_item_" + evt.target.id());
  ...
  onMouseOut(evt) {
    ...
    if (this.selectedNode) {
      this.selectedNode.style(
        "background-color",
        this.defaultNodeStyle["background-color"],
      );
  ...
  onMouseClick(evt) {
    if (this.selectedEvent) {
      const entity_id = this.selectedEvent.target.id();
      if (entity_id.startsWith(GOMODEL_PREFIX)) {
