...
export function compareTriple<EdgeMetadata>(
  a: Edge<EdgeMetadata>,
  b: Edge<EdgeMetadata>,
) {
...
export function subtractEdges<Node, EdgeMetadata>(
  ...
  const edges1 = getEdges(graph1);
  const edges2 = getEdges(graph2);
  const edges: Edge<EdgeMetadata>[] = differenceWith(
    edges1,
    edges2,
    compareTriple,
  );
...
export function subtractGraph<Node, EdgeMetadata>(
  ...
  const result: Graph<Node, EdgeMetadata> = <Graph<Node, EdgeMetadata>>{
    _nodes: {},
    _edges: {},
  };
  const keys1 = Object.keys(getNodes(graph1));
  ...
  const edgeIds: Edge<EdgeMetadata>[] = differenceWith(
    edges1,
    edges2,
    compareTriple,
  );
