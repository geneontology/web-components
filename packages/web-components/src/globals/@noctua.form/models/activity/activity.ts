...
export class Activity extends SaeGraph<ActivityNode> {
  ...
  postRunUpdateCompliment() {
    if (
      ...
      this.activityType === ActivityType.bpOnly
    ) {
  ...
  updateRootNodes() {
    ...
    mfTriples.forEach((mfTriple) => {
      ...
      if (this.activityType === ActivityType.ccOnly) {
        ...
        if (
          mfTriple.predicate.edge?.id === noctuaFormConfig.edge.locatedIn.id ||
  ...
  updateShapeMenuShex(rootTypes?) {
    each(this.nodes, (node: ActivityNode) => {
      ...
      each(
        canInsertNodes,
        (nodeDescription: ShapeDescription.ShapeDescription) => {
          insertNodes.push(nodeDescription);
        },
      );

      node.canInsertNodes = insertNodes;
      node.insertMenuNodes = filter(
        insertNodes,
        (insertNode: ShapeDescription.ShapeDescription) => {
          return true;
        },
      );
    });
  ...
  updateEdgesShex(
    ...
    insertNode: ActivityNode,
    predicate: Predicate,
  ) {
    const canInsertSubjectNodes =
      ShapeDescription.canInsertEntity[subjectNode.type] || [];
    let updated = false;

    each(
      canInsertSubjectNodes,
      (nodeDescription: ShapeDescription.ShapeDescription) => {
        if (predicate.edge.id === nodeDescription.predicate.id) {
          if (
            nodeDescription.cardinality ===
            ShapeDescription.CardinalityType.oneToOne
          ) {
            const edgeTypeExist = this.edgeTypeExist(
              subjectNode.id,
              nodeDescription.predicate.id,
              subjectNode.type,
            ...
            if (edgeTypeExist) {
              ...
              this.removeEdge(
                edgeTypeExist.subject,
                edgeTypeExist.object,
                edgeTypeExist.predicate,
              );
              this.addEdge(
                edgeTypeExist.subject,
                insertNode,
  ...
  adjustActivity() {
    if (
      this.activityType === noctuaFormConfig.activityType.options.bpOnly.name
    ) {
      ...
      if (this.bpOnlyEdge) {
        this.bpPartOfEdge.predicate.edge.id = bpNode.predicate.edge.id =
          this.bpOnlyEdge.id;
        this.bpPartOfEdge.predicate.edge.label = bpNode.predicate.edge.label =
          this.bpOnlyEdge.label;
      }
    }

    if (
      this.activityType !== ActivityType.ccOnly &&
