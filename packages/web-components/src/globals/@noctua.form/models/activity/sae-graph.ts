...
export class SaeGraph<T extends ActivityNode> {
  ...
  edgeTypeExist(
    ...
    const result = find(self.getEdges(id), (triple: Triple<T>) => {
      return (
        triple.predicate.edge.id === edgeId &&
  ...
  private _trimGraphDFS(
    ...
    if (objectNode.hasValue()) {
      const destPredicate = new Predicate(
        subjectPredicate.edge,
        objectPredicate.evidence,
      );
