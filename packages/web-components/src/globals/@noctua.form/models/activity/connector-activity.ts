...
export class ConnectorActivity extends SaeGraph<ActivityNode> {
  ...
  checkConnection(value: any) {
    ...
    self.predicate.edge = this.getCausalConnectorEdge(
      ...
      self.rule.displaySection.effectDirection && value.effectDirection
        ? value.effectDirection.id
        : null,
      self.rule.displaySection.directness && value.directness
        ? value.directness.id
        : null,
    );
    ...
    if (tree[relationship]) {
      if (tree[relationship].edge) {
        ...
        if (tree[relationship][effectDirection].edge) {
          ...
          directness &&
          tree[relationship][effectDirection][directness]
        ) {
  ...
  getCausalConnectorEdge(relationship, effectDirection, directness): Entity {
    const predicateId = this.getVPEEdge(
      relationship,
      effectDirection,
      directness,
    );
        ...
        for (const effectDirection in tree[relationship]) {
          if (tree[relationship][effectDirection].edge === edge) {
            ...
            for (const directness in tree[relationship][effectDirection]) {
              if (
                tree[relationship][effectDirection][directness].edge === edge
              ) {
  ...
  setConnectorType() {
    if (
      this.subject.activityType !== ActivityType.molecule &&
      this.object.activityType !== ActivityType.molecule
    ) {
  ...
  createGraph(srcEvidence?: Evidence[]) {
    ...
    self.addEdge(
      self.subjectNode,
      self.objectNode,
      new Predicate(self.predicate.edge, evidence),
    );
  }
  ...
  prepareSave(value) {
    ...
    const evidence: Evidence[] = value.evidenceFormArray.map(
      (evidence: Evidence) => {
        const result = new Evidence();

        result.uuid = evidence.uuid;
        result.evidence = new Entity(
