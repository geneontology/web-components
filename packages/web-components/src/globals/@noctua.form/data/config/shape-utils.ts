...
export const getTermLookup = (goCategories: GoCategory[]) => {
  ...
  if (goCategories && goCategories.length > 0) {
    ...
    termLookup = new EntityLookup(
      null,
      Object.assign({}, JSON.parse(JSON.stringify(baseRequestParams)), {
        fq: ['document_category:"ontology_class"', fqTermCategory],
      }),
    );
  }
...
export const generateBaseTerm = (
  goCategories: GoCategory[],
  override: Partial<ActivityNodeDisplay> = {},
): ActivityNode => {
  ...
  predicate.setEvidenceMeta(
    "eco",
    Object.assign({}, JSON.parse(JSON.stringify(baseRequestParams)), {
      fq: ['document_category:"ontology_class"', fqEvidenceCategory],
    }),
  );
  ...
  if (goCategories && goCategories.length > 0) {
    activityNode.termLookup = new EntityLookup(
      null,
      Object.assign({}, JSON.parse(JSON.stringify(baseRequestParams)), {
        fq: ['document_category:"ontology_class"', fqTermCategory],
      }),
