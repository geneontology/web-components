  ...
  nodes: {
    gp: <ActivityNodeDisplay>{
      ...
      category: [
        ...
        EntityDefinition.GPProteinContainingComplex,
      ],
      label: "Gene Product",
      displaySection: noctuaFormConfig.displaySection.gp,
      displayGroup: noctuaFormConfig.displayGroup.gp,
...
export const createActivityShex = (
...
export const addNodeShex = (
  activity: Activity,
  subjectNode: ActivityNode,
  predExpr: ShapeDescription.PredicateExpression,
  partialObjectNode: Partial<ActivityNode>,
): ActivityNode => {
  const overrides = getNodeDefaults(
    subjectNode,
    predExpr,
    partialObjectNode.category,
  );
  const objectNode = ShapeUtils.generateBaseTerm(
    partialObjectNode.category,
    overrides,
  );
...
export const insertNodeShex = (
  ...
  subjectNode.category.forEach((category: GoCategory) => {
    ...
    const shape = DataUtils.getRangeBySubject(
      shapes,
      category.category,
      predExpr.id,
    );
