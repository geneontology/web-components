...
export class DataUtils {
  ...
  public static getPredicates(
    shapes: ShexShapeAssociation[],
    subjectIds?: string[],
    objectIds?: string[],
    excludeFromExtension = true,
  ): string[] {
    ...
    shapes.forEach((shape) => {
      const subjectMatch =
        !subjectIds ||
        subjectIds.length === 0 ||
        subjectIds.includes(shape.subject);
      const objectMatch =
        !objectIds ||
        objectIds.length === 0 ||
        shape.object.some((objId) => objectIds.includes(objId));

      if (
        subjectMatch &&
        objectMatch &&
        shape.exclude_from_extensions !== excludeFromExtension
      ) {
        matchedPredicates.add(shape.predicate);
      }
    });
  ...
  public static getObjects(
    ...
    shapes.forEach((shape) => {
      if (
        subjectIds.includes(shape.subject) &&
        (!predicateId || shape.predicate === predicateId)
      ) {
  ...
  public static getSubjectShapes(
    shapes: ShexShapeAssociation[],
    subjectId,
    excludeFromExtensions = true,
  ): ShexShapeAssociation[] {
  ...
  public static getRangeBySubject(
    shapes: ShexShapeAssociation[],
    subjectId: string,
    predicateId: string,
  ): ShexShapeAssociation {
