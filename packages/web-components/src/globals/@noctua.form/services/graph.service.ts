...
export class NoctuaGraphService {
  ...
  loadCam(cam: Cam, removeCC: boolean = false) {
    ...
    const molecules = this.graphToMolecules(
      cam.graph,
  ...
  getNodeInfo(node) {
    ...
    each(node.types(), function (srcType) {
      const type =
        srcType.type() === "complement"
          ? srcType.complement_class_expression()
          : srcType;
    ...
    each(evidenceAnnotations, function (evidenceAnnotation) {
      ...
      if (annotationNode) {
        ...
        evidence.setEvidence(
          new Entity(nodeInfo.id, nodeInfo.label, self.getTermURL(nodeInfo.id)),
        ...
        const contributorAnnotations =
          annotationNode.get_annotations_by_key("contributor");
        const groupAnnotations =
          annotationNode.get_annotations_by_key("providedBy");
        ...
        if (sources.length > 0) {
          ...
          evidence.reference = sorted
            .map((source) => {
              return source.value();
            })
            .join("| ");
          const referenceUrl = self.getTermURL(evidence.reference);
          evidence.referenceEntity = new Entity(
            evidence.reference,
            evidence.reference,
            referenceUrl,
            evidence.uuid,
        ...
        if (withs.length > 0) {
          ...
          evidence.withEntity = new Entity(
            evidence.with,
            evidence.with,
            null,
            evidence.uuid,
  ...
  getActivityPreset(
    subjectNode: Partial<ActivityNode>,
    objectNode: Partial<ActivityNode>,
    predicateId,
    bbopSubjectEdges,
  ): Activity {
    const self = this;
    ...
    if (
      ...
      each(bbopSubjectEdges, function (subjectEdge) {
        if (
          find(noctuaFormConfig.bpOnlyCausalEdges, {
            id: subjectEdge.predicate_id(),
          })
        ) {
      ...
      objectNode.hasRootType(EntityDefinition.GoProteinContainingComplex)
    ) {
      activityType = ActivityType.proteinComplex;
    }

    return self.noctuaFormConfigService.createActivityBaseModel(
      activityType,
      subjectNode as ActivityNode,
    );
  }
  ...
  graphToActivities(camGraph): Activity[] {
    ...
    each(camGraph.all_edges(), (bbopEdge) => {
      ...
      if (self.isStartEdge(subjectNode, bbopEdge.predicate_id())) {
        ...
        const activity: Activity = self.getActivityPreset(
          subjectNode,
          objectNode,
          bbopEdge.predicate_id(),
          subjectEdges,
        );
  ...
  graphToMolecules(camGraph, visitedMFNodeIds?: string[]): Activity[] {
    ...
    for (const bbopNode of camGraph.all_nodes()) {
      ...
      if (
        subjectNode.hasRootType(EntityDefinition.GoChemicalEntity) &&
        !subjectNode.hasRootType(EntityDefinition.GoMolecularEntity)
      ) {
        ...
        if (!hasEnabledBy) {
          const activity: Activity =
            self.noctuaFormConfigService.createActivityBaseModel(
  ...
  getCausalRelations(cam: Cam) {
    ...
    each(cam.activities, (subjectActivity: Activity) => {
      each(cam.graph.get_edges_by_subject(subjectActivity.id), (bbopEdge) => {
        ...
        if (
          objectInfo.hasRootType(EntityDefinition.GoMolecularFunction) ||
          ...
          const triple = new Triple<Activity>(
            subjectActivity,
            objectActivity,
            predicate,
          );
  ...
  private _graphToActivityDFS(
    ...
    for (const bbopEdge of bbopEdges) {
      ...
      const allowedPredicate =
        this.noctuaFormConfigService.shapePredicates.find((predicate) => {
          return predicate === bbopPredicateId;
        });

      const predExpr =
        this.noctuaFormConfigService.termLookupTable[bbopPredicateId];
      ...
      if (
        activity.activityType === ActivityType.bpOnly &&
        subjectNode.term.id === noctuaFormConfig.rootNode.mf.id
      ) {
        result = [
          ...result,
          ...noctuaFormConfig.bpOnlyCausalEdges.map((edge) => edge.id),
        ];
      }
      ...
      const objectNode = this.noctuaFormConfigService.addActivityNodeShex(
        activity,
        subjectNode,
        predExpr,
        partialObjectNode,
      ...
      if (objectNode) {
        const triple: Triple<ActivityNode> = activity.getEdge(
          subjectNode.id,
          objectNode.id,
        );
        if (triple) {
          ...
          self._graphToActivityDFS(
            camGraph,
            activity,
            camGraph.get_edges_by_subject(bbopObjectId),
            triple.object,
            visitedNodeIds,
          );
        }
