...
export class GocamViewerSidebar {
  ...
  highlight(nodeId) {
    ...
    const sel = this.parentCy.elements('[id="' + nodeId + '"]');
    if (sel.size() > 0) {
  ...
  renderTaxon(gp) {
    ...
    return (
      <a href={gp.taxonURL} target="_blank">
        {label}
      </a>
    );
  }
  ...
  renderActivity(activity: Activity) {
    const nodes = activity.nodes.filter(
      (node: ActivityNode) =>
        node.predicate?.edge?.id !== "" &&
        node.predicate?.edge?.id !== noctuaFormConfig.edge.enabledBy.id,
    );

    return (
      <div
        class="activity"
        id={"gp_item_" + activity.id}
        onClick={() => this.select(activity)}
        part="activity"
      >
        <div class="gene-product" part="gene-product">
          {activity.gpNode && (
            <a href={activity.gpNode?.term.url} target="_blank">
              {activity.gpNode?.term.label}
            </a>
          )}
        </div>
        {activity.rootNode && (
  ...
  render() {
    ...
    return Object.keys(this.groupedActivities).map((process) => {
